...
const InternshipCard = ({ intern, index }) => (
  <motion.div 
    ...
    className="bg-black/30 backdrop-blur-md border border-white/10 rounded-3xl p-8 hover:border-sky-500/30 hover:bg-black/50 transition-all duration-300 flex flex-col justify-between relative group overflow-hidden shadow-2xl min-h-[450px]"
  >
    {/* Red neon side accent line revealed on hover */}
    <div className="absolute left-0 top-0 bottom-0 w-[4px] bg-blue-600 scale-y-0 group-hover:scale-y-100 origin-center transition-transform duration-500" />
    ...
    <div className="relative z-10">
      <div className="flex justify-between items-start mb-6">
        ...
        <span className="bg-white/10 text-white text-[10px] font-black tracking-widest uppercase py-1 px-3 rounded-full border border-white/15 group-hover:bg-blue-600 group-hover:text-white transition-colors duration-300">
          {intern.role.includes("QA") ? "QA Engineering" : "Development"}
        </span>
      </div>
      <h3 className="text-white text-2xl font-black mb-1 tracking-tight group-hover:text-sky-400 transition-colors duration-300">
        {intern.role}
      </h3>
    ...
    <div className="pt-4 border-t border-white/10 relative z-10">
      ...
      <div className="flex flex-wrap gap-2">
        {intern.tech.map((t) => (
          <span 
            key={t}
            className="px-3 py-1 text-xs font-mono font-bold text-white bg-white/5 rounded-full border border-white/10 hover:bg-sky-600/20 hover:border-sky-600/50 hover:text-sky-300 transition-all duration-300"
          >
            {t}
          </span>
        ))}
...
const Internships = () => {
  return (
    <section className="bg-[#00a2ff] pt-24 pb-32 px-6 md:px-12 w-full relative overflow-hidden font-sans">
