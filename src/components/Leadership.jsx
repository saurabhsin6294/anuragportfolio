...
const LeadershipItem = ({ item, index }) => {
  ...
  return (
    <div className="relative flex flex-col md:flex-row items-center justify-between mb-12 md:mb-16 w-full group">
      ...
      <div className="absolute left-4 md:left-1/2 -translate-x-1/2 w-4 h-4 bg-[#00a2ff] rounded-full border-4 border-black z-30 shadow-[0_0_15px_#00a2ff] group-hover:scale-125 group-hover:bg-white transition-all duration-300" />
      ...
      >
        <motion.div 
          ...
          whileHover={{ y: -6 }}
          className={`bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 hover:border-sky-500/30 transition-colors duration-300 relative overflow-hidden shadow-2xl w-full cursor-pointer ${
        ...
        >
          ...
          <div style={{ transform: "translateZ(20px)", transformStyle: "preserve-3d" }}>
            <div className={`flex flex-wrap gap-2 items-center mb-3 ${isEven ? 'md:justify-end' : 'md:justify-start'}`}>
              <span className="bg-[#00a2ff]/20 text-[#00a2ff] text-[10px] font-black tracking-widest uppercase py-1 px-3 rounded-full border border-[#00a2ff]/30">
                {item.badge}
              </span>
            </div>
            
            <h3 className="text-white text-xl font-black mb-1 tracking-tight group-hover:text-sky-400 transition-colors duration-300">
              {item.title}
            </h3>
            <p className="text-sky-400 text-xs font-bold font-mono tracking-wider uppercase mb-4">
              {item.role}
            </p>
  ...
  return (
    <section className="bg-[#0a0a0a] pt-24 pb-32 px-6 md:px-12 w-full relative overflow-hidden font-sans bg-[linear-gradient(to_right,#8080800a_1px,transparent_1px),linear-gradient(to_bottom,#8080800a_1px,transparent_1px)] bg-[size:80px_80px]">
      ...
      <div className="absolute top-0 left-0 w-full pointer-events-none z-10 transform -translate-y-[1px] rotate-180">
        <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className="w-full h-12 md:h-20 fill-[#00a2ff]">
      ...
      <div className="max-w-6xl mx-auto relative z-20">
        ...
        <div className="relative w-full">
          ...
          <div className="absolute left-4 md:left-1/2 -translate-x-1/2 top-2 bottom-2 w-[2px] bg-gradient-to-b from-[#00a2ff] via-red-500/50 to-white/10" />
